function ExternalBruteForce {
    ...
    #mitre_details("ExternalBruteForce")
    ...
    while ($check_file -eq $false) {
        ...
        if ($check_file) {
            ...
            Write-Host "`nPassword file: "$filename" not found. Check -`n1.If the spelling is correct`n2.If the file exists in the same directory as the tool`n3.Include extension in filename"
        }
    }
