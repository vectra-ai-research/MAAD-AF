function ExternalRecon {
    ...
    Write-Host "`nThe tool will perform reconnaissance and gather intelligence on the user and organization.." -ForegroundColor Gray
    ...
    if (Invoke-AADIntUserEnumerationAsOutsider -UserName $recon_user_name){
        Write-Host "`n[Success] Account found - Username is a valid account in the organization" -ForegroundColor Yellow
    }
    else {
        Write-Host "`n[Error] Could not verify if the account exists in organization" -ForegroundColor Red
    }
    Pause
    ...
    #Get DNS information
    Write-Host "`nGathering DNS information on the organization domain ...`n" -ForegroundColor Gray 
