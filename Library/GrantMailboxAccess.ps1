function GrantMailboxAccess{
    ...
    EnterMailbox("Enter target mailbox to gain access of")
    ...
    if ($allow_undo -eq $true) {
        ...
        if ($user_confirm -notin "No","no","N","n") {
            try {
                Write-Host "`nRemoving mailbox access to: $target_mailbox ..." -ForegroundColor Gray
                Remove-MailboxPermission -Identity $target_mailbox -User $recipient_mailbox -AccessRights FullAccess
                Write-Host "`n[Undo Success] Removed access grant to $target_mailbox mailbox" -ForegroundColor Yellow
            }
            catch {
                Write-Host "`n[Undo Error] Failed to remove access grant to $target_mailbox mailbox" -ForegroundColor Red
            }
        }
    }
    Pause
